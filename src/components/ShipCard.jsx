...
import { SHIP_STAT_LABELS, labelOf } from '../constants/display.jsx'
...
export default function ShipCard({ ship }) {
  ...
  return (
    <Link
      to={`/ships/${ship.id}`}
      className={`al-card block ${owned ? 'ring-1 ring-al-gold/30' : ''}`}
      aria-label={`${ship.name} 详情`}
    >
      ...
      <div className="grid grid-cols-4 gap-2 text-xs">
        {ship.stats && Object.entries(ship.stats).slice(0, 8).map(([key, val]) => (
          <div key={key} className="flex flex-col">
            <span className="text-al-text-dim">{labelOf(SHIP_STAT_LABELS, key)}</span>
