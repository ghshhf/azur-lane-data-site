...
> **后续迭代（同日）**：项目定位确认为**配装助手**——不会配装、不知道配装后战力、只能用已有的装备。
> 据此补上了配装台 `/fitting`，并把槽位、武器效率、舰娘面板三块改接官方档案。
> 本文件是「数据查询站」视角的首次评审，其中关于槽位口径与面板口径的描述已被后续迭代取代，
> 以 `FITTING-ENGINE-2026-09-22.md` 为准。
...
另：舰种表把 `Battlecruiser` 归入 `BB`、`Large Cruiser` 归入 `CA`，属简化口径（已收录的 24 艘里恰好只有纯正舰种，故 5 类映射全部正确）。

**逐艘实测（2026-09-22）**：把仓库里的 24 艘与 AzurAPI 官方数据逐一对名，14 艘可自动对上（其余 10 艘因改造命名、译名差异、META 舰不在官方主表而未匹配，需人工确认）。可对照的 14 艘中，**8 艘档位与官方不一致**：
...
| 亚利桑那 | SSR | Elite | 上标一档 |
| 普林斯顿 | SSR | Elite | 上标一档 |
| 西弗吉尼亚 | SSR | Elite | 上标一档 |
| 伦敦 | SSR | Elite | 上标一档 |
| 亚特兰大 | SSR | Rare | **上标两档** |
| 克利夫兰 | R | Elite | **下标两档** |
| 伯明翰 | R | Elite | **下标两档** |

关键点：**同一个官方档位 Elite，在本站被拆成 SSR（金）、R（蓝）、ELITE（橙）三种不同显示**。这不是「简化成六档」能解释的，而是三处独立录入时口径不统一。

一致的 6 艘：拉菲II、巴尔的摩、阿拉巴马、布莱默顿、格里德利，以及 META 档两艘（官方主表无 META 档，属自造档位）。

建议二选一：向官方七档 + 14 舰种对齐（需逐艘重新校对 24 艘的档位）；或明确声明为自定义简化口径并写进 README，但至少要统一 —— 目前同一档位有三种显示，用户按稀有度筛选时拿到的分组是错的。
